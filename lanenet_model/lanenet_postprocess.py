    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None, min_area_threshold=100, source_image=None):
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            ...
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                if interpolation_src_pt_y > 400:
                    point_list_y.append(interpolation_src_pt_y)
                    point_list_x.append(interpolation_src_pt_x)
                for i in range(len(point_list_y)):
                    cv2.circle(source_image, (int(point_list_x[i]),
                                              int(point_list_y[i])), 5, lane_color, -1)
        ret = {
